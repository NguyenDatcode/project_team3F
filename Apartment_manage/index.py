...
def home_page():
    ...
    if type_id:
        query = query.filter(Apartment.type_id == int(type_id))

    # Phân trang
    per_page = 5
    total = query.count()
    total_pages = (total + per_page - 1) // per_page
    apartments = query.offset((page-1)*per_page).limit(per_page).all()

    # Load dropdown
    types = ApartmentType.query.all()
...
def api_update_contract(contract_id):
    try:
        dao.update_contract(contract_id, request.form)
        return jsonify(success=True)
    except ValueError as e:
        return jsonify(success=False, message=str(e)), 400
...
@admin_bp.route("/api/regulations/<int:rid>", methods=["PUT", "DELETE"])
@login_required
def api_regulation_detail(rid):
