...
class ContractDetail(BaseModel):
    ...
    vaiTro = Column(String(50))                 # Vai trò trong hợp đồng
    ngayThamGia = Column(Date)                  # Ngày tham gia
    ngayRoiDi = Column(Date, nullable=True)    # Ngày rời đi
    trangThai = Column(String(50))             # Trạng thái người dùng
    ngayHetHan = Column(Date, nullable=True)   # Ngày hết hạn của người này trong hợp đồng

    # Quan hệ
    contract = relationship("Contract", backref="details")
    user = relationship("User", backref="contract_details")

    def __repr__(self):
        return f"<ContractDetail contract:{self.contract_id} user:{self.user_id} role:{self.vaiTro}>"
...
def load_contracts_from_json(file_path="templates/Data/hopDongThue.json"):
    ...
    for a in contracts:
        ...
        if not Contract.query.filter_by(maHopDong=a["maHopDong"]).first():
            contract = Contract(
...
def load_invoices_from_json(file_path="templates/Data/hoaDon.json"):
    ...
    for i in invoices:
        ...
        if not Invoice.query.filter_by(maHoaDon=i["maHoaDon"]).first():
            invoice = Invoice(
                maHoaDon=i["maHoaDon"],
...
def load_contracts_detail_from_json(file_path="templates/Data/chiTietHopDong.json"):
    with open(file_path, encoding="utf-8") as f:
        details = json.load(f)

    for d in details:
        contract = Contract.query.filter_by(maHopDong=d["maHopDong"]).first()
        user = User.query.get(d["id"])  # Sử dụng ID từ JSON

        if not contract or not user:
            print(f"Skipping record, contract or user not found: {d}")
            continue

        # Kiểm tra nếu ContractDetail đã tồn tại
        existing_detail = ContractDetail.query.filter_by(contract_id=contract.id, user_id=user.id).first()
        if existing_detail:
            continue

        ngayThamGia = datetime.strptime(d["ngayThamGia"], "%Y-%m-%d").date()
        ngayRoiDi = datetime.strptime(d["ngayRoiDi"], "%Y-%m-%d").date() if d.get("ngayRoiDi") else None
        ngayHetHan = datetime.strptime(d["ngayHetHan"], "%Y-%m-%d").date() if d.get("ngayHetHan") else None
    ...
    db.session.commit()
    print("Contract details loaded successfully!")
...
if __name__ == "__main__":
    with app.app_context():
        ...
        try:
            load_contracts_detail_from_json()
        except Exception as e:
            print("Error loading chiTietHopDong.json:", e)
